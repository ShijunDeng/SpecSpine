...
- Natural-language proposal generation through `specspine propose`, which deterministically turns meaningful intent into native feature spec, execution, and quality peers with EARS-like acceptance criteria, dependency-annotated tasks, and mapped quality checks.
...
- Read-only native feature analysis that reports cross-artifact consistency, task traceability, readiness blockers, and coverage evidence gaps.
- Local, deterministic agent loop packet export that combines feature summaries, readiness counts, lifecycle guidance, subagents, validation commands, safety notes, upstream metadata, and recommended commands.
...
- Status, optional status validation summaries, opt-in validation warning details, optional filterable and policy-aware feature summaries, optional workspace readiness rollups, validation, doctor, and adapter inspection commands suitable for CI and coding agents.
...
- Run `specspine status . --json` before work to understand missing artifacts, enabled upstreams, and next recommendations; add `--validate` when a single packet should also show quality-gate summary and failed check ids; use `specspine loop packet . --json` when a worker needs a local, deterministic agent loop packet before delegating implementation, validation, or review work; add `--validation-warnings` with `--validate` only when scaffold warning details are needed; add `--feature-summaries` only when choosing or comparing multiple native features; add local summary filters such as `--feature-status validated --feature-ready yes --feature-priority high --feature-sort priority` when a multi-feature workspace needs triage; add `--feature-require-coverage` when all candidate readiness must include checked local AC coverage links, or `--feature-policy` when `.specspine/policy.yaml` should decide per feature; add `--readiness-summary` when an agent or CI job needs whole-workspace ready/not-ready counts in one status packet.
- Create new natural-language requirements with `specspine propose "..." . --slug <slug>`; use `--dry-run` to preview generated Markdown, `--json` for a stable machine payload, and metadata flags for local priority, owner, milestone, target release, project, and effort context. Use `specspine feature new <slug> . --title "..." --why "..."` when a maintainer wants a manual template instead.
...
- Before implementation, run `specspine analyze . --json` to review native feature consistency, AC-to-task references, readiness blockers, and local coverage links without changing files; add `--feature <slug>` for a focused report and reserve `--fail-on-issues` for CI-style gates.
...
- `analyze --json` reports `root`, `feature_filter`, summary feature counts, issue counts by severity and category, per-feature metrics and issues, a flat issue list, and recommended commands; text output groups issues by feature and shows a clean success message when no issues exist.
- `analyze --feature SLUG` limits analysis to one valid native feature slug and reports a missing bundle as a critical local artifact issue instead of crashing; invalid slugs return code `2`.
- `analyze --fail-on-issues` is the only mode where detected issues make the command return nonzero; default analysis returns `0` when a report is built and does not write files, run tests, invoke subprocesses, probe adapters, call network services, call GitHub, read tokens, invoke upstream CLIs, or add dependencies.
- `loop packet --json` reports `root`, `deadline`, `core_features`, `summary`, `context_commands`, `lifecycle_steps`, `subagents`, `validation_commands`, `safety_notes`, `upstreams`, and `recommended_commands`; text output contains the same key sections, `--output` writes text with overwrite protection, `--deadline` is included as a string or null, and the command does not call GitHub, read or write tokens, invoke subprocesses, probe adapters, or access the network.
...
- `propose <idea>` rejects empty or punctuation-only intent with code `2`, truncates intent longer than 5000 characters with text and JSON warnings, auto-generates valid slugs when omitted, writes all three native feature files unless `--dry-run` is passed, refuses bundle conflicts unless `--force` is passed even in dry-run mode, and emits stable JSON with generated file content, warnings, and write/conflict metadata when `--json` is passed.
