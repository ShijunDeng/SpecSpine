...
- `hygiene` reports local repository hygiene by scanning generated artifacts, denylisted residue, changed-path context, and strict failure signals.
...
- Keep consistency scanning offline and advisory by linking native feature peers to local code, tests, docs, and caller-supplied changed paths without executing commands or treating references as proof that tests ran.
- Keep hygiene scanning offline and advisory by reporting generated cache artifacts, denylisted repository residue, and caller-supplied changed paths without deleting files, executing commands, or reading environment state.
...
- Keep coverage remediation planning local and read-only: reuse coverage debt and feature trace evidence to suggest review steps and quality links without writing files or treating coverage as proof of test quality.
