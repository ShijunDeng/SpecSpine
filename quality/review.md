...
- `specspine coverage plan` now turns missing AC coverage into read-only reviewer/agent remediation items with AC text, candidate test files, suggested quality links, risk notes, and focused local commands.
...
- `specspine retrospective report` now reports local feature delivery themes, readiness blockers, coverage gaps, open tasks, and deterministic recommendations without writing files or running commands.
...
- `specspine provenance manifest` now reports local SHA-256 evidence hashes and feature readiness context without printing file contents, running commands, or reading tokens.
- `specspine review packet` now composes local pre-merge review evidence from validation, quality gates, test impact, and optional feature packets without running commands or reading tokens.
...
- Use `specspine coverage plan . --json`, optionally with `--feature <slug>`, `--limit N`, or `--policy`, when coverage debt should become a reviewable remediation plan. Recommended commands are advisory and are not executed.
...
- Use `specspine retrospective report . --json`, optionally with `--feature <slug>` and `--limit N`, before planning the next iteration or after review when agents or reviewers need local feature improvement signals. Recommended commands are advisory and are not executed.
...
- Use `specspine provenance manifest . --json`, optionally with `--include PATH` and `--feature <slug>`, before review or archive when reviewers need local artifact hashes and feature evidence. Hashes prove local bytes at report time only; recommended commands are advisory.
- Use `specspine review packet . --json`, optionally with `--feature <slug>` and `--changed PATH`, before review or merge when reviewers need one local packet for validation, quality gates, test impact, feature readiness, trace, tests, gaps, blockers, and safety notes. Recommended commands are advisory and are not executed.
...
- Workspace coverage plans can now be exported as local remediation packets that keep coverage as a gap signal and never write quality links automatically.
...
- Review packets can now be exported as local pre-merge evidence bundles with validation summary, quality gate definitions, static test impact, optional feature readiness and trace evidence, review checks, safety notes, and advisory commands.
