...
- [x] Add `milestone`, `target_release`, `project`, and `effort` fields to `FeatureMetadata`.
- [x] Normalize missing assignment-style fields to `unassigned` and missing effort to `unknown`.
- [x] Generate extended metadata defaults in new feature specs.
- [x] Add extended metadata to feature summary JSON and concise text rows.
- [x] Add metadata sections to issue, task issue, PR, and sync-plan Markdown bodies.
- [x] Include metadata in handoff and test packet JSON.
- [x] Include metadata in sync-plan artifact manifests.
- [x] Add unit tests for defaults, old-file compatibility, full parsing, status summaries, drafts, artifacts, dogfood, and no remote behavior.
