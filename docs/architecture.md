...
- `specspine verify matrix` exports a local AC-level verification matrix from trace, tests, and coverage-required readiness evidence.
...
`specspine verify matrix <slug> [path] [--json]` composes trace, acceptance-test, and coverage-required readiness reports into a local acceptance-criterion verification matrix. JSON includes `root`, `feature_id`, `status`, `ready`, `matrix`, `evidence`, `summary`, `recommended_commands`, and `safety_notes`. Each row contains the AC source record, mapped test cases, mapped coverage links, coverage completeness, verification status, and gap reasons. Missing feature bundles return `1` with a structured report; invalid slugs return `2`. The command is advisory and does not run tests, invoke subprocesses, call network services, call GitHub, invoke upstream CLIs, read environment variables, or read tokens; a verified row is not proof that tests ran.
...
`specspine provenance manifest [path] [--json] [--feature SLUG] [--include PATH]...` is a local provenance and audit manifest for review, release, or archive handoffs. It hashes existing local files with SHA-256, reports file existence and byte counts, treats missing or outside-root includes as structured artifact records, and composes coverage-required feature readiness evidence when a feature slug is provided.

Provenance manifest JSON includes `root`, `feature_id`, `artifacts`, `feature_evidence`, `summary`, `recommended_commands`, and `safety_notes`. It never prints file contents; hashes prove only local file bytes at report time, not that commands or tests ran. Missing feature bundles return a report with exit code `1`; invalid slugs return `2`. The command is read-only and advisory: it does not execute tests, invoke subprocesses, call network services, call GitHub, invoke upstream CLIs, read environment variables, or read tokens.
